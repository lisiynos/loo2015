...
g++ %problem%_%author%.cpp -O2 -Wall -Wl,--stack=67108864 -std=c++11 -o fastheap.exe || goto CompSourceError
set exec=run -m 256M fastheap.exe
)
...
for %%s in (preliminary, tests\subtask1, tests\subtask2, tests\subtask3, tests\subtask4) do (
    for %%i in (%%s\??) do (
        del %problem%.out
